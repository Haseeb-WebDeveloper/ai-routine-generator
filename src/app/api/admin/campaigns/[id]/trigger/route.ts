...
async function sendEmailViaBrevo(to: string, subject: string, content: string, name?: string) {
  ...
  try {
    ...
    for (const user of users) {
      try {
        ...
        // Create direct link to quiz with secure token
        const directQuizLink = `${process.env.NEXT_PUBLIC_APP_URL}/validate?email=${encodeURIComponent(user.email)}&token=${encodeURIComponent(secureToken)}`
        ...
        await sendEmailViaBrevo(user.email, emailSubject, emailContent, user.name || 'User')
        sentCount++
